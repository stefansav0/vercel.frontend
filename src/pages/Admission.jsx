...
function Admission() {
  ...
  const fetchAdmissions = async (page) => {
    ...
    try {
      ...
      setAdmissions((prev) => {
        const combined = [...prev, ...(Array.isArray(data.admissions) ? data.admissions : [])];
      ...
      <div className="space-y-4">
        {admissions.length > 0 ? (
          admissions.map((item, index) => {
            ...
            return admissions.length === index + 1 ? (
          ...
          !error && <p className="text-center text-gray-500">No admissions available.</p>
        )}
      </div>
